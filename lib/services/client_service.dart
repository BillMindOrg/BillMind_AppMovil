...
class ClientService {
  ...
  // Actualizar un cliente por id, los datos a actualizar solo son nombre, apellido, correo y telefono
  Future<void> updateClient(int id, Client client) async {
    try {
      final response = await http
          .patch(
            Uri.parse('$baseUrl/updateInfo/$id'),
            headers: <String, String>{
              'Content-Type': 'application/json; charset=UTF-8',
            },
            body: jsonEncode(client.toJson()),
  ...
  Future<List<Debts>> getClientDebts(int clientId) async {
    ...
    if (response.statusCode == 200) {
      ...
      List<Debts> debts =
          jsonResponse.map((debt) => Debts.fromJson(debt)).toList();
      return debts;
    } else {
